...
if (typeof p5 !== 'undefined') p5.element.domify = (model, prop, replace) => this.elt.domify(model, prop, replace, this);
Element.prototype.domify = function (model, a1, a2, p5Elem) {
  ...
  let clear = typeof a1 === 'boolean' ? a1 : typeof a2 === 'boolean' ? a2 : false;
  let elt = a1 && a1.tagName ? a1 : a2 && a2.tagName ? a2 : false;
  if (elt) return elt.domify(model, prop, clear);
  if (!prop) return Object.keys(model).map(key => this.domify(model[key], key, clear));
