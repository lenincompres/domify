...
var domify = (foo, bar, atElem, useP5) => {
  ...
  var elem = (useP5 ? foo.elt : foo.tagName) ? foo : false; // is foo an elem?
  ...
  if (!elem) { // foo is not an elem
    ...
    if (!tag) tag = 'div'; // default tag for array elems
    if (IS_ARRAY) elem = foo.map(o => domify(o, [tag, ...cls].join('.'), atElem)); // elems from foo array
    else {
      elem = useP5 ? createElement(tag) : document.body.appendChild(document.createElement(tag));
      if (IS_VAL)(useP5 ? elem.elt : elem).innerHTML = foo; // no children html
  ...
  if (!IS_ARRAY) {
    ...
    if (onready) onready(useP5 ? elem.elt : elem); // passes elem to function
    return elem;
  }
};
class Bind {
