 ...
 */
 ...
 Element.prototype.domify = Element.prototype.modify = function (structure, ...args) {
   ...
   if (IS_PRIMITIVE) {
     ...
     if (['addEvent', 'addEventListener'].includes(station)) return this.addEventListener(...structure);
   } else if (IS_FUNCTION) {
   ...
   } else if (station === 'style') {
     if (IS_HEAD) return this.domify(`${Object.keys(structure).map(key => `${key} {${Object.keys(structure[key]).map(k => `${k.camelCase('-')}:${structure[key][k]}`).join(';')}`).join('}')}}`, station);
     return Object.assign(this.style, structure);
   }
