...
Element.prototype.domify = function (model, a1, a2, p5Elem) {
  ...
  if (!prop) return Object.keys(model).map(key => this.domify(model[key], key, append));
  ...
  if (IS_VALUE) {
    ...
    if (IS_HEAD && prop === 'style') return this.domify(`${Object.keys(model).map(key => `${key} {${Object.keys(model[key]).map(k => `${k.camelCase('-')}:${model[key][k]}`).join(';')}`).join('}')}}`, 'style', append);
    if (IS_ARRAY && prop === 'class') return this.classList.add(...model);
  ...
  if (!elem) {
    ...
    if (IS_ARRAY) elem = model.map(o => this.domify(o, [tag, ...cls].join('.'), append));
      ...
      else Object.keys(model).map(key => elem.domify(model[key], key, append));
    }
  }
